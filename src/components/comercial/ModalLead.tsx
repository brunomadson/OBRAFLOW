...
function ChecklistDocs({ lead }: { lead: Partial<Lead> }) {
  const [marcados, setMarcados] = useState<Record<string, boolean>>({});
  const toggle = (k: string) => setMarcados((p) => ({ ...p, [k]: !p[k] }));

  const grupos: { titulo: string; docs: DocItem[] }[] = [
    { titulo: "Documentos Pessoais (Titular)", docs: DOCS_PESSOAIS_FIXOS },
  ];
  if (lead.com_conjuge) grupos.push({ titulo: "Cônjuge",    docs: DOCS_CONJUGE });
  if (lead.dependente)  grupos.push({ titulo: "Dependente", docs: DOCS_DEPENDENTE });
  const renda = lead.tipo_renda as keyof typeof DOCS_POR_RENDA;
  if (renda && DOCS_POR_RENDA[renda]) {
    grupos.push({ titulo: DOCS_POR_RENDA[renda].label, docs: DOCS_POR_RENDA[renda].grupos });
  }

  const total     = grupos.reduce((s, g) => s + g.docs.length, 0);
  const marcTotal = Object.values(marcados).filter(Boolean).length;
  const pct       = total > 0 ? Math.round((marcTotal / total) * 100) : 0;

  return (
    <div>
      <div className="flex justify-between items-center mb-3">
        <p className="text-[13px] font-bold text-slate-700">Checklist de Documentação</p>
        <span className="text-xs font-bold text-blue-500">{marcTotal}/{total} ({pct}%)</span>
      </div>
      <div className="bg-slate-100 rounded h-1.5 mb-4">
        <div className="bg-blue-500 h-1.5 rounded transition-all duration-300" style={{ width: `${pct}%` }} />
      </div>
      {grupos.map((g) => (
        <div key={g.titulo} className="mb-4">
          <p className="text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-2">{g.titulo}</p>
          {g.docs.map((doc) => {
            const k = `${g.titulo}_${doc.id}`;
            return (
              <label key={k} className="flex items-center gap-2.5 mb-1.5 cursor-pointer">
                <input type="checkbox" checked={!!marcados[k]} onChange={() => toggle(k)}
                  className="w-3.5 h-3.5 rounded accent-blue-500" />
                <span className="text-base mr-0.5">{doc.icone}</span>
                <span className={`text-xs ${marcados[k] ? "line-through text-slate-400" : "text-slate-700"}`}>
                  {doc.label}
                  {doc.descricao && <span className="text-slate-400 ml-1">({doc.descricao})</span>}
                </span>
              </label>
            );
          })}
    ...
    <>
      <Modal onClose={onClose} size="lg">
        ...
        <div className="p-5 overflow-y-auto max-h-[52vh]">
          ...
          {tab === "docs"   && <ChecklistDocs lead={form} />}
          {tab === "editar" && (
            <FormLead
              lead={form}
              onChange={setForm}
